...
def _one_autocorr(seq, window, scale):
    ...
    :param scale: {dict} amino acid scale to be used to calculate descriptor
    :return: {list} calculated descriptor data or null-filled if too short
    """
    try:
        # Translate amino acid sequence into scale values
        m = [scale[a] for a in seq if a in scale]
        n_features = len(scale['A'])
        total_features = window * n_features

        # If the sequence is too short for the correlation window, return nulls (or zeros)
        if len(seq) <= window:
            null_values = [None] * total_features
            return null_values

        seqdesc = []
        for dist in range(window):  # for all correlation distances
            for val in range(n_features):  # for all features of the descriptor scale
                valsum = []
                cntr = 0.0
                ...
                seqdesc.append(sum(valsum) / cntr if cntr != 0 else 0)
        return seqdesc

    except ZeroDivisionError:
        # Fallback if division happens by zero (extreme edge case)
        total_features = window * len(scale.get('A', []))
        null_values = [None] * total_features
        return null_values
...
def _one_crosscorr(seq, window, scale):
    ...
    """
    try:
        # Translate sequence into numeric scale values
        m = [scale[a] for a in seq if a in scale]
        n_features = len(scale['A'])
        total_features = 0

        # Count expected descriptor length
        for val in range(n_features):
            for cc in range(n_features):
                if (val + cc) < n_features:
                    total_features += window

        # If the sequence is too short for this window, return nulls (or zeros)
        if len(seq) <= window:
            null_values = [None] * total_features
            return null_values
        ...
        n_features = len(scale.get('A', []))
        total_features = n_features * window
        null_values = [None] * total_features
        return null_values
...
class PeptideDescriptor(BaseDescriptor):
    ...
    def calculate_autocorr(self, window, append=False):
        ...
        if append:
            ...
            self.descriptor = np.array(desc)
            self.featurenames = ['Autocorr_w' + str(i + 1) for i in range(window)]
    ...
    def calculate_crosscorr(self, window, append=False):
        ...
        if append:
            ...
            self.descriptor = np.array(desc)
            self.featurenames = ['Crosscorr_w' + str(i + 1) for i in range(window)]
    ...
    def calculate_global(self, window=1000, modality='max', append=False):
        ...
        if append:
            ...
            self.descriptor = np.array(desc)
            self.featurenames = ['Global Avg']

    def calculate_all(self, window = 7):
        fn = []
        self.calculate_autocorr(window)  # Autocorrelation
        acorr = self.descriptor
        fn.extend(self.featurenames)
        self.calculate_crosscorr(window)  # Crosscorrelation
        ccross = self.descriptor
        fn.extend(self.featurenames)
        self.calculate_moment()  # Moment
        mom = self.descriptor
        fn.extend(self.featurenames)
        self.calculate_global()  # Global average
        gl = self.descriptor
        fn.extend(self.featurenames)

        self.descriptor = np.concatenate((acorr, ccross, mom, gl), axis=1)
        self.featurenames = fn
...
if __name__ == '__main__':
    ...
    parser.add_argument('-label', '--label', help='label of the sequences')
    parser.add_argument('-input', '--input', help='fasta file with proteins')
    parser.add_argument('-output', '--output', help='output file with the desired descriptor')
    ...
    if (option == 'global'):
        desc = GlobalDescriptor(input)
    elif (option == 'peptide'):
        desc = PeptideDescriptor(input)
    else:
        sys.exit()
